...
class Jenkins:
    ...
    def write_status(self, blink=False):
        ...
        for job in jobnames:
            ...
            recent = (
                time.time()
                - (
                    status[job].get("lastCompletedBuild", {}).get("timestamp", 0)
                    + status[job].get("lastCompletedBuild", {}).get("duration", 0)
                )
                / 1000
                < 180
            )
            ...
            if status[job].get("healthReport") is not None:
                scores = [
                    hr["score"]
                    for hr in status[job].get("healthReport")
                    if "score" in hr
                ]
                if scores:
                    health = min(scores)
            ...
            if status[job].get("lastCompletedBuild") is not None:
                if status[job]["lastCompletedBuild"]["result"] == "SUCCESS":
                    ...
                    raise Exception(
                        "unknown build status %s"
                        % status[job]["lastCompletedBuild"]["result"]
                    )
            ...
            if status[job].get("lastCompletedBuild", {}).get("result") == "UNSTABLE":
                ...
                teststatus += " (%d test%s failing)" % (
                    testfails,
                    "s" if testfails != 1 else "",
                )
    ...
    def run(self):
        ...
        while True:
            ...
            print(
                "%s   [%d]" % (colorama.Style.RESET_ALL + colorama.Style.DIM, iteration)
            )
