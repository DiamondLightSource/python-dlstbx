...
class DLSSchlockmeister(CommonService):
  ...
  def initializing(self):
    ...
    self._subid_watch_global = self._transport.subscribe_broadcast( \
        'ActiveMQ.Advisory.Consumer.Queue.>', self.watch_global, ignore_namespace=True)
  ...
  def _namespace_determination_failure(self):
    ...
    self.log.error('Could not determine namespace within allocated time. Shutting down.')
    self._shutdown()
  ...
  def watch_global(self, header, message):
    ...
    if self._namespace or not isinstance(message, dict):
      return
    if self._markerqueue in message.get('ConsumerInfo', {}).get('destination', {}).get('string', ''):
      self._namespace = message['ConsumerInfo']['destination']['string'][:-len(self._markerqueue)]
      ...
      self._transport.subscribe_broadcast('ActiveMQ.Advisory.Consumer.Queue.%s>' % self._namespace, self.watch_local, ignore_namespace=True)
