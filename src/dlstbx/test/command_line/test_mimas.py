...
def test_eiger_rotation(dcid, capsys):
    mimas.run([f"{dcid}", "-c"])
...
At the end of data collection {dcid}:
 ...
 - ispyb.job --new --dcid={dcid} --source=automatic --recipe=autoprocessing-xia2-3dii-eiger  --add-param=resolution.cc_half_significance_level:0.1 
 - ispyb.job --new --dcid={dcid} --source=automatic --recipe=autoprocessing-xia2-dials-eiger  --add-param=resolution.cc_half_significance_level:0.1 --trigger 
