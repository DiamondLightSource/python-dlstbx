...
class StrategyEnvironment(object):
    ...
    S_HOLD, S_PREPARE, S_STARTING, S_RUNNING, S_HOLDSHDN, S_SHUTDOWN, S_EXPIRE = range(
        S_STATUS_CODE_RANGE
    )
    ...
    def update_strategies(self, strategy_list):
        new_strategies = {
            svc["service"]: self.load_strategy(svc) for svc in strategy_list
        }
        new_launchers = {
    ...
    def update_instance(self, instance, set_alive=None):
        ...
        with self.lock:
            if host not in self.environment["instances"]:
                ...
                instance["first-seen"] = instance.get(
                    "first-seen", instance.get("last-seen", time.time())
                )
            ...
            if instance["service"] != self.environment["instances"][host]["service"]:
                ...
                del self.environment["services"][
                    self.environment["instances"][host]["service"]
                ][host]
                if instance["service"] not in self.environment["services"]:
                    self.environment["services"][instance["service"]] = {}
                self.environment["services"][instance["service"]][
    ...
    def pick_instances(
        self, instances_list, to_status, max_count, highest_fitness=True
    ):
        ordered_instances = self.order_instances(
            instances_list, reverse=highest_fitness
        )
        picked_instances = 0
    ...
    def _allocate_service(self, service):
        ...
        for instance in self.environment["services"][service]:
            existing_instances[
                self.environment["services"][service][instance]["status"]
            ].append(self.environment["services"][service][instance])
        count_instances = {x: len(existing_instances[x]) for x in existing_instances}

        instances_needed = (
            self.strategies[service]
            .assess(self.environment)
            .get("required", {})
            .get("count")
        )
        if instances_needed is None:
            ...
            self.log.debug(
                "Allocation for %s with no change required: %s",
                str(service),
                str(count_instances.values()),
            )
            return

        def log_change(count, s_from, s_to):
            self.log.debug(
                "moved %d instances of %s from %s to %s",
                count,
                str(service),
                s_from,
                s_to,
        ...
        if gap < count_instances[self.S_RUNNING]:
            ...
            picked = self.pick_instances(
                existing_instances[self.S_RUNNING],
                self.S_HOLDSHDN,
        ...
        if gap > 0 and count_instances[self.S_HOLDSHDN]:
            ...
            picked = self.pick_instances(
                existing_instances[self.S_HOLDSHDN], self.S_RUNNING, gap
            )
            count_instances[self.S_RUNNING] += picked
            count_instances[self.S_HOLDSHDN] -= picked
        ...
        if gap > 0 and count_instances[self.S_SHUTDOWN]:
            ...
            picked = self.pick_instances(
                existing_instances[self.S_SHUTDOWN], self.S_RUNNING, gap
            )
            count_instances[self.S_RUNNING] += picked
        ...
        if count_instances[self.S_PREPARE]:
            ...
            if gap <= 0:
                ...
                selected_for_removal = self.order_instances(
                    existing_instances[self.S_PREPARE], reverse=True
                )[gap:]
        ...
        if gap <= 0 and count_instances[self.S_HOLD]:
            ...
            selected_for_removal = self.order_instances(
                existing_instances[self.S_HOLD], reverse=True
            )[gap:]
        elif gap > count_instances[self.S_HOLD]:
    ...
    def update_allocation(self, queue_statistics=None):
        ...
        def log_change(instance, s_from, s_to):
            self.log.debug(
                "timer event: moved %s instance %s from %s to %s",
                str(instance.get("service", "???")),
                str(instance.get("host", "(unassigned)")),
                s_from,
                s_to,
            )
        ...
        with self.lock:
            ...
            for key, instance in self.environment["instances"].iteritems():
                ...
                if (
                    instance["status"] == self.S_HOLD
                    and instance["status-set"] + hold_time < current_timestamp
                ):
                    instance["status"] = self.S_PREPARE
                    instance["status-set"] = time.time()
                    log_change(instance, "HOLD", "PREPARE")
                if (
                    instance["status"] == self.S_HOLDSHDN
                    and instance["status-set"] + hold_time < instance["last-seen"]
                ):
                ...
                if (
                    instance["status"]
                    not in (self.S_HOLD, self.S_PREPARE, self.S_EXPIRE)
                    and instance["last-seen"] < current_timestamp - expiration_time
                ):
                    log_change(
                        instance, "running state (%d)" % instance["status"], "EXPIRE"
                    )
                    instance["status"] = self.S_EXPIRE
                    instance["status-set"] = time.time()
                if (
                    instance["status"] == self.S_EXPIRE
                    and instance["status-set"] < current_timestamp - discarding_time
                ):
    ...
    def balance_services(self, callback_start=None, callback_stop=None):
        ...
        with self.lock:
            ...
            if available_for_reassignment:
                self.log.debug(
                    "Instances available for assignment: %s",
                    str(available_for_reassignment),
                )
            ...
            if startup and callback_start:
                for key in startup:
                    if callback_start(
                        ...
                        self.launchers.get(
                            self.environment["instances"][key]["service"]
                        ),
                    ):
