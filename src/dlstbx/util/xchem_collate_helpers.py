...
def export_pipedream_files(
    ...
    if refined_pdb_file.exists() and output_cif_file.exists():
        # copyfile, not copy2: the destinations are usually owned by gda2, and
        # copy2's copystat calls os.utime, which needs ownership rather than
        # just write access. Writing the bytes is fine via the visit-group ACL,
        # so anyone but gda2 running this hit EPERM part-way through the export.
        shutil.copyfile(refined_pdb_file, target_pdb)
        shutil.copyfile(output_cif_file, target_cif)
