...
class Topaz3Wrapper(Wrapper):
    ...
    def run(self):
        ...
        assert "results_directory" in params, (
            "Could not find results directory in parameters"
        )
        assert "topaz_python" in params, (
        ...
        assert os.path.exists(params["working_directory"]), (
        ...
        assert os.path.exists(params["model_file"]), (
        ...
        if "rgb" in params:
            assert isinstance(params["rgb"], bool), (
        ...
        assert "original_phase_file" in payload, (
            "Could not find original phase file in payload"
        )
        assert "inverse_phase_file" in payload, (
        ...
        assert isinstance(payload["space_group"], str), (
            f"Expected string for space group, got {payload['space_group']}"
        )
        assert isinstance(payload["best_solvent"], str), (
        ...
        assert os.path.exists(params["results_directory"]), (
