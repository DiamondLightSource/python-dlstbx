...
class GraylogRRD():
  ...
  def setup_rrd(self):
    ...
    self.rrd_graylog = self.rrd.create(
          'graylog-message-summary.rrd', [ '--step', '60' ]
  ...
  def update(self):
    ...
    update_from = max(self.rrd_graylog.last_update + 60, update_time - 30 * 24 * 3600)
    ...
    for datapoint in sorted(data):
      ...
      update_record.extend(data[datapoint].get(level, 0) for level in sorted(loglevels))
