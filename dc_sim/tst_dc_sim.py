...
def test_check_should_return_NONE_if_some_successful_processing_results_are_available():
  db, result = make_dummy_db_and_test_dictionary(
      'i03',
      'native',
      { 2960726: [ (p, True) for p in all_programs[:2] ] },
  )
...
def test_check_should_return_PASS_if_all_processing_results_are_good():
  db, result = make_dummy_db_and_test_dictionary(
      'i03',
      'native',
      { 2960726: [ (p, True) for p in all_programs ] },
  )
...
def test_check_should_return_FAIL_if_all_processing_results_are_bad():
  db, result = make_dummy_db_and_test_dictionary(
      'i03',
      'native',
      { 2960726: [ (p, False) for p in all_programs ] },
  )
...
def test_check_should_return_FAIL_if_one_processing_result_is_bad():
  broken_program = 3
  broken_results = [ (p, n != broken_program) for n, p in enumerate(all_programs) ]
  random.shuffle(broken_results)

  db, result = make_dummy_db_and_test_dictionary(
      'i03',
      'native',
      { 2960726: broken_results },
  )
...
# assert all_programs[broken_program] in result['reason']
...
def test_check_should_return_FAIL_if_a_single_processing_result_is_bad():
  ...
  db, result = make_dummy_db_and_test_dictionary(
      'i03',
      'native',
      { 2960726: [ (all_programs[broken_program], False) ] },
  )
