...
class S3EchoUploader(CommonService):
    ...
    def on_upload_dcid(self, rw, header, message):
        """
        Upload images from DLS filesystem to S3 Echo object store.
        """
        # Conditionally acknowledge receipt of the message
        ...
        # set in environment to list of uploaded file names in the message that
        # are prefixed with processingjobid value.
        s3echo_upload_files_all = rw.environment.get(
            "s3echo_upload", {}
        ) or message.get("s3echo_upload", {})
        for dcid, s3echo_upload_files in s3echo_upload_files_all.items():
            upload_file_list = s3echo_upload_files.keys()
            if (
                s3_urls := message.get("s3_urls", {})
                ...
                try:
                    ...
                    if len(s3_urls) < len(s3echo_upload_files):
                        rw.checkpoint(
                            {
                                "s3_urls": s3_urls,
                                "s3echo_upload": s3echo_upload_files_all,
                            },
                            delay=self._message_delay,
                            transaction=txn,
                        )
    ...
    def on_download(self, rw, header, message):
        ...
        if s3_urls := params.get("cleanup", True) and rw.environment.get("s3_urls"):
