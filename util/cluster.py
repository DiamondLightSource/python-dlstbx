...
class Cluster:
       ...
       command line. Cache the results."""
        ...
        for variable in ("DRMAA_LIBRARY_PATH", "SGE_ROOT", "PATH"):
            if not environment.get(variable):
                raise RuntimeError(
                    "Could not load cluster environment, required variable %s unset"
                    % variable
                )
    ...
    def _qstat(self, jobid):
        ...
        decodestatus = {
            self.drmaa.JobState.UNDETERMINED: "process status cannot be determined",
    ...
    def _qwait(self, jobid):
        ...
        print(
            "Job: {0} finished with status {1}".format(retval.jobId, retval.hasExited)
        )
...
class ClusterStatistics:
    ...
    def parse_job_xml(self, j):
        ...
        if job["state"] == "running":
            job["start"] = self.convert_string_to_time(
                j.getElementsByTagName("JAT_start_time")[0].firstChild.nodeValue
            )
            job["age"] = self.age(job["start"])
            job["queue"], job["node"] = (
                j.getElementsByTagName("queue_name")
                or j.parentNode.getElementsByTagName("name")
            )[0].firstChild.nodeValue.split("@")
        if job["state"] == "pending":
            job["submission"] = self.convert_string_to_time(
                j.getElementsByTagName("JB_submission_time")[0].firstChild.nodeValue
            )
    ...
    def parse_queue_xml(q):
        ...
        queue["slots_used"] = int(
            q.getElementsByTagName("slots_used")[0].firstChild.nodeValue
        )
        queue["slots_reserved"] = int(
            q.getElementsByTagName("slots_resv")[0].firstChild.nodeValue
        )
        queue["slots_total"] = int(
            q.getElementsByTagName("slots_total")[0].firstChild.nodeValue
        )
        queue["slots_free"] = (
        ...
        queue["alarm"] = (
            any(char in queue["state"] for char in "aA") and queue["enabled"]
        )
        if q.getElementsByTagName("load-alarm-reason"):
            queue["alarm-reason"] = q.getElementsByTagName("load-alarm-reason")[
                0
            ].firstChild.nodeValue
        if q.getElementsByTagName("message"):
            queue["message"] = q.getElementsByTagName("message")[0].firstChild.nodeValue
        queue["suspended"] = (
            any(char in queue["state"] for char in "sSC")
            or queue["unknown"]
    ...
    def summarize_node_status(node):
        ...
        for queue in node:
            summary[queue["class"]] = {
                "slots": queue["slots_total"],
                "used": queue["slots_used"],
                "reserved": queue["slots_reserved"],
            }
            if queue["error"]:
