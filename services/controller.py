...
class DLSController(CommonService):
    ...
    def initializing(self):
        ...
        self._status_subscription_id = str(
            self._transport.subscribe_broadcast(
                "transient.status", self.receive_status_msg, retroactive=True
            )
        )

        # Listen to queue status reports for information on queue utilization.
        self._transport.subscribe_broadcast(
            "transient.queue_status", self.receive_queue_status
        )
    ...
    """
        ...
        def recognize_synchronization_message(header, message):
            if (
                self._sync_subscription_id
                and header.get("subscription") == self._sync_subscription_id
            ):
                # Synchronization message detected
                self.master_last_checked = time.time()
            if (
                self._sync_subscription_id
                and header.get("subscription") == self._status_subscription_id
            ):
    ...
    def survey_operations(self):
       ...
       everything is working fine and resources are deployed appropriately."""
        ...
        if not self.master:
            return
        if self.master_since == 0 or self.master_since > time.time() - 30:
        ...
        self._se.balance_services(
            callback_stop=self.kill_service, callback_start=self.start_service
        )
            ...
            with self._lock:
                try:
                    ...
                    if self.timestamp_strategies_loaded is None:
                        ...
                        self._sync_subscription_id = str(
    ...
    def receive_status_msg(self, header, message):
        ...
        if instance[
            "wfstatus"
        ] == CommonService.SERVICE_STATUS_STARTING and message.get("tag"):
            self._se.register_instance_tag_as_host(
                message["tag"], instance["host"], instance["service"]
            )
       ...
       a fixed time interval."""
        ...
        for queue in self._se.watched_queues():
            qstat = self._jmx.org.apache.activemq(
                ...
                attribute=",".join(
                    ("QueueSize", "EnqueueCount", "DequeueCount", "InFlightCount")
                ),
            )
    ...
    def start_service(self, instance, init, soft_fail=False):
        ...
        for attempt in init:
            try:
                ...
                self.log.info(
                    "Failed to start %s with %s, error: %s",
                    service,
                    str(attempt),
                    str(e),
                    exc_info=True,
    ...
    def kill_service(self, instance):
        self.log.info(
            "Shutting down instance %s (%s)",
            instance["host"],
            str(instance.get("title")),
        )
        self._transport.broadcast(
            "command", {"host": instance["host"], "command": "shutdown"}
        )
        return True
