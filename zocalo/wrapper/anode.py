...
class AnodeWrapper(zocalo.wrapper.BaseWrapper):
    def run(self):
        ...
        if not os.path.exists(pdb_file):
            logger.info(
                "Not running anode as required input pdb file %s does not exist"
                % pdb_file
            )
            return

        shutil.copyfile(pdb_file, os.path.join(working_directory, "anode_input.pdb"))
    ...
    def run_shelxc(self, sca_file, working_directory):
        ...
        stdin = "\n".join(
            ["SAD %s" % sca_file, "CELL %s" % cell_str, "SPAG %s" % sg_str, "MAXM 2"]
        )
