...
def parse_file(filename):
    ...
    for test in x.getElementsByTagName("testcase"):
        trace_candidate = test.getElementsByTagName(
            "failure"
        ) or test.getElementsByTagName("error")
        if trace_candidate:
...
def run():
    files = glob.glob(
        "/dls/science/groups/scisoft/DIALS/RHEL7VM/workspace/dials_bootstrap_python3/tests/*.xml"
    )
    for f in files:
        print(f)
        print(parse_file(f))
