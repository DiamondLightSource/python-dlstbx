...
class Xia2MultiplexWrapper(zocalo.wrapper.BaseWrapper):
  ...
  def run(self):
    ...
    if scaled_unmerged_mtz.check():
      ...
      merging_stats = get_merging_statistics(i_obs.customized_copy(anomalous_flag=False))
      anom_merging_stats = get_merging_statistics(i_obs.customized_copy(anomalous_flag=True))
      ...
      for stats, bin in (('overall', merging_stats.overall),
      ...
      for stats, bin in (('overall', anom_merging_stats.overall),
